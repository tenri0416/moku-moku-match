...
<div class="hidden md:block min-h-screen bg-[#F8FAFF] text-[#071433]">
    <div class="mx-auto w-full max-w-[1440px] px-8 py-8">
        ...
        <div class="grid grid-cols-[250px_1fr_310px] gap-6">
            ...
            <aside class="space-y-5">
                {{-- 募集作成 --}}
                <section
                    class="rounded-[16px] bg-emerald-500 px-5 py-6 text-white shadow-[0_12px_24px_rgba(16,185,129,0.22)]">
                    <div
                ...
                <section
                    ...
                    <div class="space-y-2">
                        @foreach ($quickFilterLinks as $filter)
                            @php
                                $isActive =
                                    count($filter['params']) === 0
                                        ? !request()->hasAny(['purpose', 'location_type', 'time_zone'])
                                        : collect($filter['params'])->every(
                                            fn($value, $key) => request($key) == $value,
                                        );
                            @endphp
                            ...
                            <a href="{{ route('home', $filter['params']) }}"
                                ...
                                <span
                                    class="flex h-8 w-8 items-center justify-center rounded-full bg-[#F1F5F9] text-[17px]">
                                    {{ $filter['icon'] }}
                                </span>
                                {{ $filter['label'] }}
                            </a>
            ...
            <main class="min-w-0 space-y-5">
                ...
                <section>
                    ...
                    <form method="GET" action="{{ route('home') }}"
                        class="mb-4 grid grid-cols-[1fr_1fr_1fr_1fr_110px] gap-3">
                        <div>
                    ...
                    <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between gap-4">
                                    <div class="min-w-0 flex-1">
                                        <div class="mb-2 flex items-center gap-2">
                                            <span
                                                class="rounded-full bg-blue-50 px-3 py-1 text-[12px] font-black text-[#0D4FE8]">
                                                {{ $purposeLabel }}
                                            </span>

                                            <span
                                                class="rounded-full bg-[#F1F5F9] px-3 py-1 text-[12px] font-black text-[#334155]">
                                                {{ $locationLabel }}
                                            </span>
                                        </div>
                                        ...
                                        <div
                                            class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 text-[13px] font-bold text-[#64748B]">
                                            <span>🗓️
                                                {{ optional($workPost->start_at)->format('Y/m/d') ?? optional($workPost->created_at)->format('Y/m/d') }}</span>
                                            <span>{{ optional($workPost->start_at)->format('H:i') ?? '08:00' }}〜{{ optional($workPost->end_at)->format('H:i') ?? '10:00' }}</span>
                        ...
                        @empty
                            <div
                                class="rounded-[14px] border border-dashed border-[#CBD7EA] bg-white px-5 py-8 text-center">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                @foreach ($homeArticles as $article)
                                    @php
                                        ...
                                        $articleTitle = $article->h1_title ?? ($article->seo_title ?? $article->title);
                                    @endphp
            ...
            <aside class="space-y-5">
                ...
                <section
                    ...
                    <div
                        class="mb-5 grid grid-cols-2 overflow-hidden rounded-[8px] border border-[#CBD7EA] bg-[#F8FAFF] p-1">
    ...
    {{-- フッター --}}
    <footer class="mt-8 border-t border-[#DDE6F5] pt-6">
        <div
            class="flex flex-col items-center justify-between gap-4 rounded-[16px] bg-white px-6 py-5 shadow-[0_8px_22px_rgba(15,43,95,0.06)] md:flex-row">
            <div>
                <p class="text-[15px] font-black text-[#071433]">
                    MokuMoku Match
                </p>
                <p class="mt-1 text-[12px] font-bold text-[#64748B]">
                    リモートワーク・勉強の作業仲間が見つかるマッチングサービス
                </p>
            </div>

            <div class="flex items-center gap-4">
                <a href="{{ url('/privacy-policy') }}"
                    class="text-[13px] font-black text-[#64748B] transition hover:text-[#0D4FE8]">
                    プライバシーポリシー
                </a>

                <span class="h-4 w-px bg-[#DDE6F5]"></span>

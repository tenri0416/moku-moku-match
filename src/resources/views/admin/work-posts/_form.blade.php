    ...
    @enderror
        ...
        <div>
            ...
            <select
                ...
                class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            >
                <option value="1" @selected(old('status', $workPost->status ?? 1) == 1)>募集中</option>
                <option value="2" @selected(old('status', $workPost->status ?? '') == 2)>終了</option>
                <option value="3" @selected(old('status', $workPost->status ?? '') == 3)>非公開</option>
            </select>

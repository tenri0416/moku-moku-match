...
@php
    ...
    $workPostTitle = $workPostTitle ?? ($workPost->title ?? '直接メッセージ');

    $messageStoreUrl = Route::has('messages.users.store')
        ? route('messages.users.store', $user)
        : route('messages.store', [$workPost, $user]);

    $profileUrl = Route::has('users.profiles.show')
        ? route('users.profiles.show', $user)
        : (Route::has('users.show') ? route('users.show', $user) : '#');
@endphp
                        ...
                        @if ($hasBlocked)
                            <div class="rounded-[14px] border border-rose-200 bg-rose-50 px-5 py-5">
                                ...
                                <div class="mt-4 flex justify-end gap-3">
                                    <a
                                        href="{{ route('messages.index') }}"
                                        class="flex h-[46px] items-center justify-center rounded-[12px] border border-[#CBD7EA] bg-white px-5 text-[14px] font-black text-[#071433]"
                                    >
                                        一覧へ戻る
                                    </a>
                                    ...
                                    <a
                                        href="{{ $profileUrl }}"
                                        class="flex h-[46px] items-center justify-center rounded-[12px] bg-rose-600 px-5 text-[14px] font-black text-white"
                                    >
                                        プロフィールで解除する
                                    </a>
                                </div>
